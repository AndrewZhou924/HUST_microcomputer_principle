...
; 更改数码管的显示图案
; 先判断COFFSET是否为44，若是则跳过无需修改   
BANKSEL COFFSET     
MOVF COFFSET,0     
SUBWF .44
BTFSC STATUS,Z 
GOTO CASE1_LAST ; COFFSET此时是44
BANKSEL STOP_WATCH ; COFFSET此时不是44  
MOVF STOP_WATCH,0
SUBLW .10
...
;若此时KEY_FLAG==0，则表示此前没按下或为双击，需跳过双击判断
BANKSEL KEY_FLAG    
BTFSS KEY_FLAG,0
GOTO CASE2_RESET_KEY_STATE_0 ; KEY_FLAG == 0
;KEY_FLAG != 0   
...
BANKSEL KEY_STATE    
MOVLW .0
MOVWF KEY_STATE ; 给KEY_STATE赋值为0    
    
; 判断KEY_VALUE == KEY_VALUE_LAST？    
BANKSEL KEY_VALUE     
MOVF KEY_VALUE,0 
BANKSEL KEY_VALUE_LAST      
SUBWF KEY_VALUE_LAST,0
...
SUBWF KEY_VALUE_LAST,0
BTFSS STATUS,Z     
GOTO CASE3_ERROR ; Z == 0, KEY_VALUE != KEY_VALUE_LAST      
CASE3_NORMAL    
BANKSEL KEY_FLAG    
BTFSS KEY_FLAG,0
GOTO MAIN ; KEY_FLAG == 0，直接进入下一步循环
...
CASE3_ERROR ; kenneg发生了错误操作，如按下7的同时按下8
;检查秒表
;GOTO CASE3_NORMAL    
    
    
;先把KEY_STATE置位0，再重置图案
BANKSEL KEY_STATE    
MOVLW .0  ; STOP_WATCH > T3，判为长按
MOVWF KEY_STATE  
...
; z == 1, KEY_STATE == 3
MOVLW .15 ; 重置 STOP_WATCH为T2+1，使得下次被按下不会被判断为双击
